...
theorem sb_injective (hf : Injective f) (hg : Injective g) : Injective (sbFun f g) := by
  ...
  rw [A_def] at x₂A
  calc
    x₁ = g (invFun g x₁) := by rw [sb_right_inv f g x₁A]
    _ = g (invFun g x₂) := by congr
    _ = x₂ := by rw [sb_right_inv f g x₂A]
...
theorem sb_surjective (hf : Injective f) (hg : Injective g) : Surjective (sbFun f g) := by
  ...
  · rw [A_def, sbSet, mem_iUnion] at gyA
    ...
    exact hg hx
  have : g y ∉ sbSet f g := by
    rw [← A_def]
    exact gyA
  use g y
  calc
    h (g y) = invFun g (g y) := by
      simp only [h_def, sbFun, if_neg this]
    _ = y := by
      apply hg
      apply invFun_eq
      use y
